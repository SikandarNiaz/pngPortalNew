...
@Component({
  selector: 'app-survey-shops-surveyor',
  templateUrl: './survey-shops-surveyor.component.html',
  styleUrls: ['./survey-shops-surveyor.component.scss']
})
export class SurveyShopsSurveyorComponent implements OnInit {
  title = 'Survey Merchandiser List';
  ...
  getMerchandiserList() {
    ...
    this.httpService.getSurveySurveyorList(obj).subscribe((data: any) => {
  ...
  gotoNewPage(item) {
   ...
   window.open(`${environment.hash}dashboard/survey_shop_list?surveyorId=${item.surveyor_id}&startDate=${this.startDate}&endDate=${this.endDate}`, '_blank');
